...
if (class_exists('Klick_Apv_Abstract_Notice')) return;

/**
 * Class Klick_Apv_Abstract_Notice
 */
abstract class Klick_Apv_Abstract_Notice {
	...
	/**
	 * Klick_Apv_Abstract_Notice constructor
	 */
	...
	public function do_notice() {
		...
		return klick_apv()->get_dashboard()->include_template('notices-templates/' . $this->notice_template_file, false, $notice_data);
	}
	...
	public function available_at_this_time() {
		...
		$display_notice_time = klick_apv()->get_options()->get_option('notice-display-time');
		...
		if (!isset($display_notice_time[$this->notice_id]) || time() > $display_notice_time[$this->notice_id]) {
				...
				klick_apv()->get_options()->update_option('notice-display-time', $display_notice_time);
				return true;
		} else {
				return false;
		}
	...
	public function is_plugin_configured($plugin) {
		...
		return ($plugin === "advanced-plugin-view/advanced-plugin-view.php" && klick_apv()->get_options()->get_option('send-url'));
	}
}
